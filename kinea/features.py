...
def default_feature_recipes(config: Config) -> tuple[FeatureRecipe, ...]:
    """Forecasting defaults: HICP YoY and point-in-time EUR/CZK month-to-date mean."""
    recipes = []
...
def _calculate(points: list[tuple[str, float, str]], recipe: FeatureRecipe) -> FeatureValue | None:
    ...
    if recipe.transform == "level":
        value = latest
    elif len(points) <= recipe.periods:
        return None
    elif recipe.transform == "pct_change":
        previous = points[-1 - recipe.periods][1]
        if previous == 0:
            return None
        value = (latest / previous - 1.0) * 100.0
    else:
        value = latest - points[-1 - recipe.periods][1]
