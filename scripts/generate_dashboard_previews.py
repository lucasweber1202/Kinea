...
def as_of_preview(metadata, demo_current, demo_history, old):
    ...
    fig.text(0.94, 0.055, "Generated from evidence/revision_demo.db", ha="right", fontsize=9, color="#98A2B3")
...
def hicp_preview(metadata, current):
    ...
    fig.text(0.045, 0.905, "Monthly Czech price-index levels · ECB Data Portal · native values",
             fontsize=12, color="#667085")
    ...
    ax.set_title("Full available history", loc="left", fontsize=14, weight="bold", color=NAVY, pad=14)
    ...
    table_ax.axis("off")
    table_ax.text(0, 1.02, "Latest observations", fontsize=14, weight="bold", color=NAVY,
                  transform=table_ax.transAxes)
    rows = [[name, row.reference_date.date(), f"{row.value:.2f}", row.vintage_date.date()]
            for name, row in latest_rows]
    table = table_ax.table(cellText=rows, colLabels=["Component", "Period", "Index", "Vintage"],
                           colWidths=[0.36, 0.24, 0.16, 0.24], cellLoc="left", colLoc="left",
                           loc="upper left", bbox=[0, 0.42, 1, 0.48])
    table.auto_set_font_size(False)
    ...
    table_ax.text(0, 0.08, "Period selection and component filters are available in Streamlit.",
                  fontsize=9, color="#667085", wrap=True)
    fig.text(0.94, 0.055, "HICP section · evidence/kinea.db", ha="right", fontsize=9, color="#98A2B3")
...
def fx_preview(metadata, current):
    ...
    fig.text(0.045, 0.905, "Czech koruna per euro · daily ECB reference rate · raw published level",
             fontsize=12, color="#667085")
    ...
    fig.text(0.06, 0.095, "A higher value means a weaker koruna against the euro.",
             fontsize=13, weight="bold", color=NAVY)
    fig.text(0.94, 0.055, "EUR/CZK section · evidence/kinea.db", ha="right", fontsize=9, color="#98A2B3")
...
def vintages_preview(metadata, demo_history):
    ...
    detail = demo_history[(demo_history.series_id == series_id)
                          & (demo_history.reference_date == reference)].sort_values("vintage_date")
    first, last = detail.iloc[0], detail.iloc[-1]
    fig = plt.figure(figsize=(16, 9), dpi=120, facecolor="#F6F8FB")
    fig.text(0.045, 0.94, "Vintage history", fontsize=27, weight="bold", color=NAVY)
    fig.text(0.045, 0.905, "Old and current values coexist; a later revision never destroys prior knowledge",
             fontsize=12, color="#667085")
    fig.patches.append(FancyBboxPatch((0.045, 0.76), 0.91, 0.105, transform=fig.transFigure,
        boxstyle="round,pad=0.01,rounding_size=0.012", facecolor="#FFFAEB", edgecolor="#FEDF89"))
    fig.text(0.065, 0.825, "Labelled simulated revision", fontsize=13, weight="bold", color="#B54708")
    fig.text(0.065, 0.785,
             "Stored in evidence/revision_demo.db; official ECB values in evidence/kinea.db are untouched.",
             fontsize=11, color="#B54708")
    ax = fig.add_axes([0.07, 0.27, 0.36, 0.38], facecolor="white")
    bars = ax.bar([str(value.date()) for value in detail.vintage_date], detail.value,
                  color=["#98A2B3", BLUE], width=0.52)
    ax.set_ylim(detail.value.min() - 0.25, detail.value.max() + 0.25)
    ax.set_title("Both vintages retained", loc="left", fontsize=14, weight="bold", color=NAVY, pad=14)
    ax.set_ylabel("Index", color="#667085")
    ax.grid(axis="y", color="#E7ECF2")
    ax.spines[["top", "right", "left"]].set_visible(False)
    ...
    ax_table.axis("off")
    ax_table.text(0, 1.02, f"{labels[series_id]} · {reference.date()}",
                  fontsize=14, weight="bold", color=NAVY, transform=ax_table.transAxes)
    rows = [["Old", first.vintage_date.date(), f"{first.value:.2f}", "+0.00"],
            ["Current", last.vintage_date.date(), f"{last.value:.2f}", f"{last.value-first.value:+.2f}"]]
    table = ax_table.table(cellText=rows, colLabels=["Version", "Vintage date", "Value", "Difference"],
                           cellLoc="left", colLoc="left", loc="upper left", bbox=[0, 0.45, 1, 0.40])
    table.auto_set_font_size(False)
    ...
    ax_table.text(0, 0.18, "Append a new vintage dated 18 July 2026", fontsize=16,
                  weight="bold", color=NAVY)
    ax_table.text(0, 0.06, "Composite key: (series_id, reference_date, vintage_date)",
                  fontsize=10, color="#667085")
    fig.text(0.94, 0.055, "Vintages section · evidence/revision_demo.db", ha="right", fontsize=9, color="#98A2B3")
...
def audit_preview(metadata, current, logs):
    ...
    fig.text(0.045, 0.905, "One complete log per execution · success and error paths preserved",
             fontsize=12, color="#667085")
    ...
    _card(fig, 0.505, 0.76, 0.205, 0.105, "SUCCESS LOGS", f"{(logs.status == 'success').sum()}", BLUE)
    ...
    ax.axis("off")
    ax.text(0, 1.05, "Execution log", fontsize=14, weight="bold", color=NAVY, transform=ax.transAxes)
    rows = []
    ...
    table = ax.table(cellText=rows, colLabels=["ID", "Status", "Started", "Finished", "Summary"],
                     colWidths=[0.05, 0.09, 0.19, 0.19, 0.48], cellLoc="left", colLoc="left",
                     loc="upper left", bbox=[0, 0.05, 1, 0.82])
    table.auto_set_font_size(False)
    ...
    for (row, col), cell in table.get_celld().items():
        ...
        if row == 0:
            ...
            cell.set_text_props(weight="bold", color=TEAL if cell.get_text().get_text() == "SUCCESS" else RED)
    fig.text(0.06, 0.22, "Reproduce validation", fontsize=11, color="#667085")
    fig.text(0.06, 0.17, "python scripts/validate_delivery.py", fontsize=16, family="monospace",
             weight="bold", color=NAVY)
    fig.text(0.06, 0.11, "Expected result: all checks PASS, followed by READY", fontsize=11, color="#475467")
    fig.text(0.94, 0.055, "Audit section · evidence/kinea.db", ha="right", fontsize=9, color="#98A2B3")
